...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{../src/outputs/figures/tweet_length_distribution.png}
    \caption{Distribution of tweet lengths across sentiment classes.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../src/outputs/figures/cm_w2v_logreg.png}
        \caption{W2V + Logistic Regression}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \includegraphics[width=\textwidth]{../src/outputs/figures/cm_w2v_rf.png}
        \caption{W2V + Random Forest}
    \end{subfigure}
    \caption{Confusion matrices for Word2Vec-based models.}
    \label{fig:cm_w2v}
\end{figure}
...
\textbf{FinBERT} & \textbf{0.8685} & \textbf{0.8300} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{Macro F1-score comparison across all models.}
...
This work demonstrates the effectiveness of transformer-based models for financial tweet sentiment classification. Our best model, \textbf{FinBERT}, achieves \textbf{86.85\% accuracy} and a \textbf{macro F1-score of 0.83} on the three-class sentiment task.
...
\begin{itemize}[noitemsep]
    \item \textbf{Back-translation augmentation}: Translating minority class tweets to an intermediate language (e.g., French, German) and back to English generates semantically equivalent paraphrases, synthetically expanding the Bearish training set without introducing label noise.
    \item \textbf{Advanced augmentation strategies}: Contextual word replacement using masked language models (replacing tokens with BERT-predicted alternatives), sentence-level paraphrasing with generative models, and mixup in embedding space.
    \item \textbf{Extended pre-training (domain-adaptive)}: Continuing FinBERT's pre-training on our specific tweet corpus with masked language modeling (MLM) objective would adapt the model to Twitter-specific language patterns, abbreviations, and financial slang not present in formal financial text.
    \item \textbf{Focal Loss}: Replacing weighted cross-entropy with Focal Loss ($\gamma=2$) to dynamically down-weight easy Neutral examples and focus learning capacity on ambiguous Bearish/Bullish cases.
    \item \textbf{Ensemble methods}: Combining FinBERT predictions with TF-IDF + LogReg via stacking or voting, leveraging complementary signal from lexical and contextual representations.
    \item \textbf{Multi-task learning}: Jointly predicting sentiment and topic (from BERTopic labels) to encourage richer intermediate representations.
\end{itemize}
